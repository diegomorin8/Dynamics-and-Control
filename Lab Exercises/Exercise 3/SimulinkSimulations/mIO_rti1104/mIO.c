...
void mIO_output(void)
{
  ...
  switch ((int32_T)mIO_P.CtrlSignalSelector_Value) {
   ...
   default:
    mIO_B.MultiportSwitch = mIO_B.Quantizer;
    break;
  }
...
void mIO_derivatives(void)
{
  ...
  _rtXdot->Integrator_CSTATE = mIO_B.Sum4;
...
RT_MODEL_mIO_T *mIO(void)
{
  ...
  /* Initialize timing info */
  {
    ...
    mIO_M->Timing.sampleTimes[1] = (0.001);
  ...
  mIO_M->Timing.stepSize0 = 0.001;
  mIO_M->Timing.stepSize1 = 0.001;
  mIO_M->solverInfoPtr = (&mIO_M->solverInfo);
  mIO_M->Timing.stepSize = (0.001);
  rtsiSetFixedStepSize(&mIO_M->solverInfo, 0.001);
  ...
  mIO_M->Sizes.numBlockIO = (78);      /* Number of block outputs */
