...
%% - Specifications 
    ...
    w0          = 2*abs(pdVec(1));
% outstr=strcat('Specifications found                    [x];   (','wN=  ',num2str(double(wn)),' | zeta=  ',num2str(zeta),' | w0=  ',num2str(double(w0)),')  ');disp(outstr);                                                                                        %     freq_param = vpa([wn, zeta],2);display('    wn   |   zeta');display(freq_param);
    [Zeros,Poles,Gain] = zpkdata(G);
    
display('Finding desired Poles complete          [x];');
%% - Angels
...
%% - Controller Type
    display('Choosing controller type:');
%%
